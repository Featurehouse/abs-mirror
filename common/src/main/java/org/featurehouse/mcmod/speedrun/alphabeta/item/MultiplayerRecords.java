...
public class MultiplayerRecords {
    ...
    public static int invite(@NotNull ServerPlayerEntity self, @NotNull Collection<? extends ServerPlayerEntity> players,
        ...
        final Invitation invitation = new Invitation(self.getUuid(), uuid,
                RecordSnapshot.fromRecord(rec, Objects.requireNonNull(self.getServer()).getOverworld().getTime()).asText(),
        ...
        final Text text0 = invitation.toText(Objects.requireNonNull(self.getServer()).getPlayerManager());
        ...
        invitationCaches.stream().filter(c -> invited.getUuid().equals(c.invitedPlayer())).findAny()
                .ifPresentOrElse(c -> {
                    if (accept) {
                        ...
                        RecordSnapshot record1 = RecordSnapshot.fromRecord(coopRecord, Objects.requireNonNull(host.getServer()).getOverworld().getTime());
