...
public final class JsonReader implements Closeable {
	...
	public JsonToken peek() throws IOException {
		...
		return switch (p) {
			...
			case PEEKED_NUMBER, PEEKED_HEXADECIMAL, PEEKED_NaN, PEEKED_INF, PEEKED_NEGATIVE_INF -> JsonToken.NUMBER;
	...
	public Number nextNumber() throws IOException {
		...
		if (p == PEEKED_NUMBER) {
			...
			checkLenient(); // using this to catch where this is used, quilt_TODO remove
			if (p == PEEKED_UNQUOTED) {
	...
	int doPeek() throws IOException {
		...
		switch (c) {
			case ']':
				// if we're at the end of an array but we peeked a comma before, then allow only on json5
				if (peekStack != JsonScope.EMPTY_ARRAY) {
					assertJson5();
				}
				return peeked = PEEKED_END_ARRAY;
...
//						checkNotStrict(); // fall-through
	...
	private void checkLenient() throws IOException {
		throw syntaxError("This file may be valid in lenient GSON, but it is not valid JSON5");
	}
