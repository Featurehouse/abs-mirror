...
public class AlphabetSpeedrunModImpl {
    public static Collection<? extends ItemSpeedrunDifficulty> fireDifficultyRegistry() {
        return FabricLoader.getInstance().getEntrypoints(DifficultiesFactory.FABRIC_ENTRYPOINT_NAME, DifficultiesFactory.class)
                .stream().flatMap(f -> f.registerDifficulties().stream()).toList();
    }
}
